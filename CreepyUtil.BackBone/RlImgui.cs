...
public static partial class RlImgui
{
    ...
    public static void SetScale(float scale = 1) => Scale = GetIO().FontGlobalScale = scale;
    ...
    public static byte[] LoadCascadiaCode(out int byteLength)
    {
        ...
        var stream =
            asm.GetManifestResourceStream("CreepyUtil.BackBone.Resources.AddedAssets.Font.CascadiaMono.ttf");
    ...
    {
        var posX = GetCursorPosX() + GetColumnWidth()
                   - CalcTextSize(text).X;
    ...
    {
        var posX = GetCursorPosX() + GetColumnWidth() / 2f
                   - CalcTextSize(text).X / 2f;
